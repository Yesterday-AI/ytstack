...
- [ ] **`ytstack-skill-check` CI tool.** Validator for UX contracts (frontmatter, checklist, flow graph, preamble flags, AskUserQuestion format, em-dash punctuation). Referenced across docs. **Now Phase-1 priority -- agents/ addition + post-M009 audit surfaced real conformance gaps that a validator would have caught.** (Note: banned-words list rolled back 2026-04-24 as scope-creep; only em-dash rule is mechanical.)
...
- [x] **Greenfield-flow first-skill is wrong (2026-04-24).** IMPLEMENTED 2026-04-24. Changes shipped: init-project refactored to infra-only (scope question only; PM questions removed; PROJECT.md populated from OFFICE-HOURS.md frontmatter when present). office-hours is now the greenfield entry (its `description:` states it first). plan-ceo-review + plan-eng-review got dual-mode (concept + milestone) via auto-detection of pitch vs milestone artifacts. using-ytstack directive trimmed of trigger-map + greenfield-flow section; selection now runs on skill-description semantic matching per DECISIONS 2026-04-24 "Skill selection is semantic, not keyword-based". Verified via live smoke-test in `/tmp/.../ytstack-smoke-readme-link-check`: greenfield prompt "baue mir eine cli..." auto-routed to office-hours, pitch produced, plan-ceo-review concept-mode annotated. Docs updated: README Quickstart, QUICKSTART.md, docs/concept.md §5.1.
